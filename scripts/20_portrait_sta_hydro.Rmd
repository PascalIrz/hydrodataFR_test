...
Débit observé le `r dern_obs$date_obs %>% format('%d/%m/%Y')` à `r dern_obs$date_obs %>% lubridate::hour()`h`r dern_obs$date_obs %>% lubridate::minute()` : `r dern_obs$debit` $m^3/s$.
...
g_htr <- ggplot(data = data,
  ...
  geom_col(aes(y = em), stat = "identity", width = 20000, alpha = 0.15, fill = "lightblue") +
    geom_line() +
  scale_x_datetime(date_labels = "%d/%m") + # étiquette axe des dates
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = "", y = bquote('Débit ('*m^3/s*')')) + # légendes des axes
  geom_hline(yintercept = syn_data$mm$`Module (moyenne) est`,
             col = "firebrick4", linetype = "dashed") +
  geom_hline(yintercept = syn_data$be %>% 
                          .["Quinquennale seche",] %>%
                          pull(`QMNA (m3/s) est`),
             col = "red")
