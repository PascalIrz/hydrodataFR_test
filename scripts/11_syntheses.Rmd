...
La machine de l'utilisateur est supposée configurée avec les outils nécessaires pour compiler des *packages* R. Ce qui permet d'installer le package {hydrodataFR}.
...
Les préfixes des noms de cette famille de fonctions correspondent à `bh` pour "Banque HYDRO" et `sy` pour "Synthèse".
...
C'est un peu fouillis, en apparence du moins. En effet, on peut accéder par exemple aux écoulements mensuels de cette station :
...
ggplot(data = donnees, aes(x = `Débit (m3/s)`, y = frequence)) +
...
>Une fois les données chargées est structurées, l'utilisateur peut en faire ce qu'il veut.
